...
export function FixedHorizontalScrollbar({ targetRef }: FixedHorizontalScrollbarProps) {
  ...
  return (
    <div
      className="fixed bottom-0 z-50 border-x border-t border-slate-300 bg-white/95 px-1 pt-1 shadow-[0_-4px_14px_rgba(15,23,42,0.12)] backdrop-blur"
      style={{ left: metrics.left, width: metrics.width }}
      aria-label="Rolagem horizontal da tabela"
    >
      <div
        ref={scrollbarRef}
        className="fixed-horizontal-scrollbar h-5 overflow-x-scroll overflow-y-hidden"
        onScroll={(event) => {
          if (syncingRef.current || !targetRef.current) return;
          syncingRef.current = true;

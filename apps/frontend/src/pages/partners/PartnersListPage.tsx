...
export default function PartnersListPage() {
  ...
  function update(patch: Partial<PartnerFilters>) {
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        title="Parceiros"
        description={`${total} parceiro${total === 1 ? '' : 's'} cadastrado${total === 1 ? '' : 's'} na rede comercial.`}
        actions={
          canCreate && (
            <button className="btn-primary" onClick={() => navigate('/partners/novo')}>
      ...
      <div className="filter-panel">
        <div className="grid gap-3 md:grid-cols-[1fr_220px_180px]">
          ...
          <select
            ...
            value={filters.type ?? ''}
            onChange={(e) =>
              update({ type: (e.target.value || undefined) as PartnerFilters['type'] })
            }
          >
          ...
          <select
            ...
            onChange={(e) =>
              update({ status: (e.target.value || undefined) as PartnerFilters['status'] })
            }
          >
      ...
      <div className="table-shell overflow-x-auto">
        {loading ? (
          <p className="p-12 text-center text-sm text-slate-400">Carregando parceiros…</p>
        ) : (
          <table className="data-table">
            <thead>
              <tr>
                <th>
                  <Header field="name">Parceiro</Header>
                </th>
                <th>
                  <Header field="type">Tipo</Header>
                </th>
                <th>
                  <Header field="contactName">Contato</Header>
                </th>
                <th>
                  <Header field="leads">Oportunidades</Header>
                </th>
                <th>Comissão</th>
                <th>
                  <Header field="status">Status</Header>
                </th>
                ...
                <tr key={partner.id}>
                  <td>
                    ...
                    >
                      ...
                      <p className="mt-0.5 text-xs text-slate-400">
                        {partner.document || partner.legalName || 'Documento não informado'}
                      </p>
                    </button>
                  </td>
                  <td>
                    <span className="rounded-lg bg-purple-50 px-2.5 py-1 text-xs font-semibold text-brand-purple">
                  ...
                  <td>
                    ...
                    <p className="text-xs text-slate-400">
                      {partner.email || formatPhone(partner.phone)}
                    </p>
                  </td>
                  <td>
                    <button
                  ...
                  <td>
                    <span
                      className={`inline-flex items-center gap-2 text-xs font-semibold ${partner.status === 'ACTIVE' ? 'text-emerald-600' : 'text-slate-400'}`}
                    >
                      <span
                        className={`status-dot ${partner.status === 'ACTIVE' ? 'bg-emerald-500' : 'bg-slate-300'}`}
                      />
